                    ...
                    <div class="flex justify-between mt-2 ml-4">
                        <div class="flex-1">
                          <div class="flex flex-col">
                              <div class="">
                                  <div class="text-left pr-1 text-bold text-lg text-blue-800 m-0 leading-3">Tỉnh/thành <span class="m-0 font-normal text-base font-italic text-gray-400">(Province)</span></div>
                              </div>
                              <div class="flex-1 ">
                                <Multiselect
                                  class="rounded-sm h-6 w-full "
                                  v-model="form.province_id"
                                  v-bind="provincesSelect"
                                  >
                                </Multiselect>
                              </div>
                          ...
                          <div class="grid grid-cols-5 mr-2">
                              ...
                              <div class="col-span-3">
                                    <select
                                      ...
                                      <option v-for="(wd, i) in getwards" :key="i" :value="wd.code">{{wd.name}} - ({{wd.code}})</option>
                                    </select>
                              </div>
                          </div>
                        </div>
              ...
              <!----Info Goldgate================================================----->
                      <div class="grid grid-cols-8 border-2 border-blue-500 rounded-md">
                        <div class=" col-span-5 border-r border-blue-500 ">
                            ...
                            <div class="col-span-2">Bác sỹ đọc mẫu</div>
                              <div class="col-span-4 h-8">
                                <select
                                    ...
                                    >
                                    <option v-for="(rd, i) in readcodes" :key="i" :value="rd.read_code">
                                      <span v-if ="rd.sub_read_code == null">{{rd.name}} - ({{rd.read_code}})</span>
                          ...
                          <div class="flex">
                              <span class="w-48 pl-2">Đọc mẫu: </span>
                                <div class=" flex-1">
                        ...
                        <div class="col-span-3">
                            ...
                            <div class="flex mt-4 px-2">{{hpv_code_edit}}
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      provincesSelect: {
        ...
        value: "value",
        label: "name",
        options: this.provinces_input.data,
        searchable: true,
        createTag: true,
      },
      districtSelect: {
  ...
  watch: {
    "form.chooseReadCode":function(value){

      this.getsubReadCode(value)
  ...
  methods: {
    changeCode(){
      setTimeout(function(){
      alert(123);
      this.test = 'this.thin_code_last';
      },4000)
    },
    async getsubReadCode(data) {
 ...
 //alert(JSON.stringify(fillReadcodeSub));
     return this.readcodeSub = fillReadcodeSub;
    },
    async getReadCodeEdit(value) {

     return this.readCodeEdit = "value";
    },
    async checkAll() {
      if (!this.allSelected) {
    ...
    updateBill(bill) {
      ...
      const dataupdate = {
        ...
        thin_code_edit:'form.selectThin'
      };
    ...
    getDistrictFill(province) {
      ...
      var fillData = this.districts_input.data.filter(function (el) {
        return el.province_id == province;
      });
//       const resultArray = Object.keys(fillData).map(function(key) {
//   return [Number(key), fillData[key]];
// });
