<template>
  <app-layout title="User list">
    ...
    <Container>
      ...
      --->
      <Card>
           <Button  class="mb-1 float-right cursor-pointer"  @click="addLaboGroup">+ Add</Button>
        <Table :headers="headers" :addClass="addClass">
          <tr class="hover:bg-blue-300" v-for="(gr,i) in labogroups.data" :key="i">
        ...
        <div class="mt-4">
         <div class="flex">
            <template v-for="(link, i) in labogroups.links" :key="i" >
        ...
        <DialogModal :show="showModal" class="mb-0 pb-0 bg-green-700" :bgHeader="editMode ? bgEdit : bgSave">
             <template v-slot:title >
               <div class="flex justify-between">
                <h3 v-show="!editMode">Thêm nhóm xét nghiệm </h3>
                <h3 v-show="editMode">Chỉnh sửa nhóm xét nghiệm</h3>
...
data(){
  ...
  methods:{
    ...
    updateLaboGroup(data){
       data._method = 'PUT';
        this.$inertia.post('/dashboard/labogroups/'+data.id, data)
        this.reset();
        this.closeModal();
    }
