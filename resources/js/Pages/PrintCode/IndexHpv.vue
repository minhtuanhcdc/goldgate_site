          ...
          <div class="">
            <div class="" v-for="(b,i) in billtests" :key="i">
             <template v-for="i in 3" :key="i.index">
              <div class="frame_print text-center border-1 border-gray-500">
                <span class="name_print font-bold text-xs">{{b.custommer.name.split(/<[/\w\s-]+>|\s/g).pop()}} - {{b.custommer.birthday}}</span>
                <p class="badcode_print -my-1 flex justify-center" v-if="b.hpv_code">
                  <vue-barcode class="h-6 w-28" :value="b.hpv_code" :options="{ displayValue: false }"></vue-barcode>
                </p>
                <p v-else></p>
                <span class="code_print font-bold z-40 text-xs">{{b.hpv_code}}</span>
              </div>
