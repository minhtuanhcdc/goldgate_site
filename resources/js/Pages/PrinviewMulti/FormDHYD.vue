      ...
      <div class="grid grid-cols-3 font-sans-Timenew leading-4 fontsizeInfo ">
       ...
       <div class="text-left font-bold fontsizeName  font-sans-Timenew mt-2 text-blue-800">LOAI BỆNH PHẨM <span class="pl-5 text-red-600 mr-0 pr-0">Tế bào học nhúng dịch ThinPrep</span>
                    ...
                    <div class="" v-for="eg11 in testElements" :key="eg11.id">
                     <span class="font-bold pr-4" v-if="eg11.element_group ==11">
                          <!-- <span v-if="eg11.id == 27" >
                              <input type="checkbox" disabled class="form-checkbox text-blue-800 h-3 w-3" :checked="td.results.map(({result }) =>result)[0] !=='28'" />
                          </span>
                          <span v-else>
                            <input type="checkbox"   class="ml-1 my-1 form-checkbox text-blue-800 h-3 w-3" :checked="td.results.map(({result }) =>result)[0] ==='28'" />
                          </span>-->
                          <span>
              ...
              <div class="" v-for="(eg12,i) in testElements" :key="i">
                     <div class="font-bold fontsizeInfo leading-3 mr-4" v-if="eg12.element_group ==12">
                      <span v-if="eg12.id == 29">
                              <!-- <span v-if="td.results.map((x) =>+x.element_id).findIndex((x) => 29 !== x) ">
                                <input type="checkbox" disabled class="form-checkbox text-blue-800 h-3 w-3 leading-4" checked/>
                              </span> -->
                              <span>
        ...
        <div class="grid grid-cols-3">
           ...
           <div class="grid grid-cols-1 text-center fontsizeInfo">
             <span class="font-bold font-sans-Timenew ">Ngày đọc kết quả: {{currentDate}}</span>
             <span class=" font-bold font-sans-Timenew fontsizeName" style="margin-bottom:60px">BÁC SĨ ĐỌC KẾT QUẢ</span>

              <span class="font-bold font-sans-Timenew" v-if="td.doctorread">{{td.doctorread.title}}<span class="ml-1"> {{td.doctorread.name}}</span></span>
           </div>
           </div>
