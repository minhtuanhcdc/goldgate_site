    ...
    <Container>
      <Card>
        ...
        <Table :headers="headers" :addClass="addClass">
          <tr v-for="(or, i) in oureads.data" :key="i" class="hover:bg-blue-300 align-middle">
            ...
            <td class="border-r-2 w-56">
              <div v-if="or.asign" class="flex">
                <img :src="pathAsign + or.asign" class="h-10 w-28"/>
                <DeleteX
                    :url="route('deleteAsign', or.id)"
                    class="w-4 p-0 cursor-pointer text-red-700"
                    :moduleName="'asign of' + `${or.name}`"
                  />
              </div>
              <div v-else></div>
            </td>
            ...
            <div class="px-4 pb-0">
                ...
                <div class="grid grid-cols-1">
                 ...
                 <div class="ml-2 border-1 border-gray-700">
                     <div>
                      <input type="file" class="hidden" ref="asignImage" @change="updatePhotoAsign()" />
                      <jet-label for="header" value="Hình chữ ký" class="text-blue-800"/>
                      <!-- Current Profile Photo   @change="updatePhotoPreview"-->
                      <div class="mt-2" v-show="!asignPreview">
                        <img :src="pathAsign + imageAsign" class="h-16 mx-auto" />
                      </div>
                      <!-- New Profile Photo Preview -->
                      <div class="mt-2" v-show="asignPreview">
                         <span class="w-full  justify-center">
                        <img :src="asignPreview" class="mx-auto  h-16" />
                        </span>
                      </div>
                      <jet-secondary-button
                        class="mt-2 mr-2 bg-blue-600 rounded-lg text-white py-1 px-2 cursor-pointer"
                        type="button"
                        @click.native.prevent="selectNewAsign"
                      >
                        Chọn Hình Chữ ký
                      </jet-secondary-button>

                      <jet-input-error :message="errorMessage" class="mt-2" />
                      <span class="ml-4 py-1 px-2 cursor-pointer bg-yellow-600 rounded-lg" @click="removeAsignImage()">Remove Asign</span>
                    </div>
                  </div>
                <div class="pl-2 mt-4">
                  <input
...
data(){
  ...
  methods:{
     updatePhotoAsign() {
      ...
      this.form.asign =  this.$refs.asignImage.files[0];
      };
      reader.readAsDataURL(this.$refs.asignImage.files[0]);
    },
     selectNewAsign() {
      this.$refs.asignImage.click();
    },
    removeAsignImage() {
    ...
    editOuRead(or) {
        ...
        this.editMode = true;
        this.imageAsign = or.asign;
       if (or.asign_hpv == 1) this.asignHpv = true;
      else this.asignHpv = false;
